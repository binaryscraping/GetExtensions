...
/// A retry policy that retries requests using an exponential backoff for allowed HTTP methods and
/// HTTP status codes
/// as well as certain types of networking errors.
public struct Retrier: APIClientDelegate {
  ...
  /// See [RFC 2616 - Section 9.1.2](https://www.w3.org/Protocols/rfc2616/rfc2616-sec9.html) for
  /// more information.
  public static let defaultRetryableHTTPMethods: Set<HTTPMethod> = [
    .delete,
    // [Delete](https://www.w3.org/Protocols/rfc2616/rfc2616-sec9.html#sec9.7) - not always idempotent
    .get,
    // [GET](https://www.w3.org/Protocols/rfc2616/rfc2616-sec9.html#sec9.3) - generally idempotent
    .head,
    // [HEAD](https://www.w3.org/Protocols/rfc2616/rfc2616-sec9.html#sec9.4) - generally idempotent
    .options,
    // [OPTIONS](https://www.w3.org/Protocols/rfc2616/rfc2616-sec9.html#sec9.2) - inherently idempotent
    .put,
    // [PUT](https://www.w3.org/Protocols/rfc2616/rfc2616-sec9.html#sec9.6) - not always idempotent
    .trace,
    // [TRACE](https://www.w3.org/Protocols/rfc2616/rfc2616-sec9.html#sec9.8) - inherently idempotent
  ]

  /// The default HTTP status codes to retry.
  /// See [RFC 2616 - Section 10](https://www.w3.org/Protocols/rfc2616/rfc2616-sec10.html#sec10) for
  /// more information.
  ...
  public static let defaultRetryableURLErrorCodes: Set<URLError.Code> =
    [ // [Security] App Transport Security disallowed a connection because there is no secure
      // network connection.
      //   - [Disabled] ATS settings do not change at runtime.
      // .appTransportSecurityRequiresSecureConnection,
      // [System] An app or app extension attempted to connect to a background session that is
      // already connected to a
      // process.
      //   - [Enabled] The other process could release the background session.
      .backgroundSessionInUseByAnotherProcess,

      // [System] The shared container identifier of the URL session configuration is needed but has
      // not been set.
      ...
      // .badURL,
      // [System] A connection was attempted while a phone call is active on a network that does not
      // support
      ...
      // [File System] A download task couldn’t create the downloaded file on disk because of an I/O
      // failure.
      //   - [Disabled] File system error is unlikely to recover with retry.
      // .cannotCreateFile,
      ...
      // [Data] Content data received during a connection request could not be decoded for a known
      // content encoding.
      //   - [Disabled] Server is unlikely to modify the content encoding during a retry.
      // .cannotDecodeRawData,
      ...
      // [Security] A client certificate was required to authenticate an SSL connection during a
      // request.
      ...
      //   - [Disabled] File system error is unlikely to recover with retry.
      // .fileDoesNotExist,
      // [File System] A request for an FTP file resulted in the server responding that the file is
      // not a plain file,
      // but a directory.
      //   - [Disabled] FTP directory is not likely to change to a file during a retry.
      // .fileIsDirectory,
      // [Network] A redirect loop has been detected or the threshold for number of allowable
      // redirects has been
      // exceeded (currently 16).
      //   - [Disabled] The redirect loop is unlikely to be resolved within the retry window.
      // .httpTooManyRedirects,
      // [System] The attempted connection required activating a data context while roaming, but
      // international roaming
      // is disabled.
      //   - [Enabled] WiFi connection could be established during retry.
      .internationalRoamingOff,

      // [Connectivity] A client or server connection was severed in the middle of an in-progress
      // load.
      ...
      // [Connectivity] A network resource was requested, but an internet connection has not been
      // established and
      // cannot be established automatically.
      //   - [Enabled] A network connection could be established during retry.
      .notConnectedToInternet,

      // [Resource] A redirect was specified by way of server response code, but the server did not
      // accompany this
      ...
      // .resourceUnavailable,
      // [Security] An attempt to establish a secure connection failed for reasons that can’t be
      // expressed more
      // specifically.
      //   - [Enabled] The secure connection could be established during a retry given the lack of
      //   specificity
      ...
      //   - [Disabled] The server certificate is unlikely to become trusted within the retry
      //   window.
      ...
      //   - [Disabled] The error could not be interpreted and is unlikely to be recovered from
      //   during a retry.
      // .unknown,
      ...
      // [Resource] A server reported that a URL has a non-zero content length, but terminated the
      // network connection
  ...
  ///   - retryableHTTPStatusCodes: The HTTP status codes that are automatically retried by the
  /// policy.
  ///                               `RetryPolicy.defaultRetryableHTTPStatusCodes` by default.
  ///   - retryableURLErrorCodes:   The URL error codes that are automatically retried by the
  /// policy.
  ///                               `RetryPolicy.defaultRetryableURLErrorCodes` by default.
  public init(
    retryLimit: UInt = Retrier.defaultRetryLimit,
    ...
    retryableHTTPMethods: Set<HTTPMethod> = Retrier.defaultRetryableHTTPMethods,
  ...
  public func client(
    ...
    if attempts < retryLimit, shouldRetryTask(task, dueTo: error) {
      let delay = pow(Double(exponentialBackoffBase), Double(attempts)) * exponentialBackoffScale
  ...
  private func shouldRetryTask(_ task: URLSessionTask, dueTo error: Error) -> Bool {
    guard
      let httpMethod = task.currentRequest?.httpMethod,
    ...
    return retryableURLErrorCodes.contains(errorCode)
  }
}
